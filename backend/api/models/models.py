...
class Match(Base):
    ...
    max_distance = Column(Float, default=5.0, nullable=False)  # 最大距離
...
class MatchUser(Base):
    ...
    user_start_lat = Column(Float, nullable=False)  # ユーザーの開始位置（緯度）
    user_start_lng = Column(Float, nullable=False)  # ユーザーの開始位置（経度）
    user_destination_lat = Column(Float, nullable=False)  # ユーザーの目的地（緯度）
    user_destination_lng = Column(Float, nullable=False)  # ユーザーの目的地（経度）
