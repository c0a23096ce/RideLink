  ...
  export default function LobbyView() {
    ...
    const handleSearch = async () => {
      ...
      if (data && data.length > 0) {
        ...
        focus === 'origin' ? setOrigin([lat, lon]) : setDestination([lat, lon])
      } else {
        alert('場所が見つかりませんでした')
      }
    }
    ...
    return (
      <Box flex={1} display="flex" flexDirection="column" justifyContent="center" alignItems="center" height="100vh">
        <Box display="flex" gap={4}>
          <Button variant="outlined" sx={{ borderRadius: 1 }} onClick={openCreateDialog}>
            ⚡ マッチングロビーの作成
          </Button>
          <Button variant="outlined" sx={{ borderRadius: 1 }} onClick={openJoinDialog}>
            ⚡ マッチングへの参加
          </Button>
        </Box>
